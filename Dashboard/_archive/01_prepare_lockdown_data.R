...
# TODO:
# 1. Global. Can select 1/2/3/4 any containment or specific one
# 2. Show diff-in-diff coefficient for each country-keyboard
...
# Load Data --------------------------------------------------------------------
## Lockdown Data
ox_df <- readRDS(file.path(dropbox_file_path, "Data", "oxford_covid_policy_tracker", "RawData", "OxCGRT_latest.Rds"))

## Date of first 1/2/3/4 for each category. Show in table.
...
a <- ox_df[ox_df$CountryName %in% "United States",]
...
gtrends_df <- gtrends_df[gtrends_df$keyword_en %in% KEYWORDS_CONTAIN_USE,]
gtrends_df <- gtrends_df %>%
  dplyr::select(geo, date, keyword, keyword_en, hits_ma7)
