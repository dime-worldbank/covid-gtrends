...
cor_6_df <- readRDS(file.path(dropbox_file_path, "Data", "google_trends", "FinalData",
  ...
  dplyr::mutate(timespan = "2022-07-01 to 2022-12-31")
...
cor_df <- bind_rows(cor_1_df,
  ...
  mutate(keyword_en = keyword_en %>% 
         ...
         timespan = timespan %>% factor(levels = c("2022-07-01 to 2022-12-31",
...
#### Order Keywords
cor_avg_df <- cor_df %>%
  dplyr::filter(timespan %in% c("2020-01-01 to 2020-06-30")) %>%
  group_by(keyword_en) %>%
