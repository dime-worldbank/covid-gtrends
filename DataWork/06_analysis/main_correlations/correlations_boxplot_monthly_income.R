...
gtrends_monthly_df <- gtrends_monthly_df %>%
  dplyr::select(geo, keyword_en, year, cor_excess, cor_cases, income) %>%
...
p <- gtrends_monthly_df %>%
  dplyr::filter(income != "Not classified",
                keyword_en %in% c("Loss of Smell",
                                  "Loss of Taste",
                                  "Covid Symptoms")) %>%
  dplyr::mutate(income = income %>%
                  factor(levels = c("High income",
                                    "Upper middle income",
                                    "Lower middle income",
                                    "Low income"))) %>%
...
ggsave(p, filename = file.path(paper_figures, "cases_excess_boxplot_income.png"),
       height = 6, width = 7)
